...
def upload_jsonfile():
    ...
    if file_path:
        with open(file_path, 'r', encoding="utf-8") as json_file:
            ...
            with open("vault.txt", "a", encoding="utf-8") as vault_file:
                for chunk in chunks:
                    # Write each chunk tx1o its own line
                    vault_file.write(chunk.strip() + "\n")  # Two newlines to separate chunks
            print(f"JSON file content appended to vault.txt with each chunk on a separate line.")
def upload_dir_of_jsons():
    # ask the user to select a directory, after it, find all json files, upload_jsonfile() for each
    directory = filedialog.askdirectory()
    for filename in os.listdir(directory):
        if filename.endswith(".json"):
            upload_jsonfile(os.path.join(directory, filename))
...
# Create a button to open the file dialog for a directory of JSON files
json_dir_button = tk.Button(root, text="Upload Directory of JSON Files", command=upload_dir_of_jsons)
json_dir_button.pack(pady=10)

# Run the main event loop
root.mainloop()
# say vscode hotkey to reformat Code on mac
